...
from ...db.modelle import Auftrag, AuftragProtokoll, Video
...
class AuftragKontext:
    ...
    def __init__(self, auftrag_id: str, video_id: str | None, werte: dict[str, Any], art: str) -> None:
    ...
    async def protokoll(self, text: str, stufe: str = "info") -> None:
        async with sitzung() as s:
            ...
            await s.commit()
        bus.veroeffentliche("auftrag_protokoll", auftrag_id=self.auftrag_id, video_id=self.video_id, stufe=stufe, text=text)
        (log.warning if stufe == "fehler" else log.info)("[%s %s] %s", self.art, (self.video_id or "")[:8], text)
...
class Laeufer:
    ...
    async def _naechste(self, s: AsyncSession, art: str, anzahl: int, werte: dict[str, Any]) -> list[Auftrag]:
        # Ein Video hat höchstens einen laufenden Auftrag (Stufen bauen aufeinander auf).
        laufende_videos = {self._kontexte[k].video_id for k in self._laufend if self._kontexte[k].video_id}
        q = (
            select(Auftrag)
        ...
        aus: list[Auftrag] = []
        for a in rows:
            if a.video_id and a.video_id in laufende_videos:
                continue
            aus.append(a)
            if a.video_id:
                laufende_videos.add(a.video_id)
            if len(aus) >= anzahl:
                break
        return aus
    ...
    def _starte(self, a: Auftrag, werte: dict[str, Any]) -> None:
        k = AuftragKontext(a.id, a.video_id, werte, a.art)
        ...
        bus.veroeffentliche("auftrag_status", auftrag_id=a.id, video_id=a.video_id, art=a.art, status=Auftragsstatus.LAEUFT)
...
async def auftrag_anlegen(
    s: AsyncSession, art: Auftragsart, video_id: str | None, prioritaet: int = 0, parameter: dict[str, Any] | None = None
) -> Auftrag | None:
    """Legt einen Auftrag an, wenn für dieses Video und diese Art keiner wartet oder läuft."""
    if video_id is not None:
        ...
        if offen:
            return None
    a = Auftrag(art=art, video_id=video_id, prioritaet=prioritaet, parameter=parameter or {})
    s.add(a)
    await s.flush()
    bus.veroeffentliche("auftrag_status", auftrag_id=a.id, video_id=video_id, art=art, status=a.status)
    return a
