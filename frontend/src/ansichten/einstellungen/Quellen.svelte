<script lang="ts">
  ...
  let neu = $state({ typ: "tubevault", name: "", basis_url: "http://192.168.178.49:8031", kanal_id: "" });
  ...
  async function kanalPruefen(): Promise<void> {
    ...
    kanal = null;
    try {
      kanal = await api.post<KanalAusgabe>("/quellen/pruefen", { typ: neu.typ, basis_url: neu.basis_url, kanal_id: neu.kanal_id });
  ...
  async function anlegen(): Promise<void> {
    beschaeftigt = true;
    try {
      await api.post<Quelle>("/quellen", neu);
      meldungen.gut("Quelle angelegt");
      neu = { typ: "tubevault", name: "", basis_url: neu.basis_url, kanal_id: "" };
      kanal = null;
      await laden();
    } catch (e) {
...
<section class="m-ansicht">
  <div class="m-ansicht-kopf">
    <h1>Quellen</h1>
    <span class="m-unter">Woher die Videos kommen</span>
    <InfoKnopf anker="erste-schritte" />
  </div>
  <div class="m-ansicht-koerper">
        ...
        <div class="card-body">
          <div class="d-flex align-items-start gap-3 flex-wrap">
            <div class="flex-grow-1">
              ...
              <div class="text-secondary">{q.basis_url} &middot; Kanal {q.kanal_id}{#if q.kanal_name} ({q.kanal_name}){/if}</div>
              ...
              <div class="small text-secondary mt-1">
                ...
                {q.regeln.nur_heruntergeladene !== undefined && q.regeln.nur_heruntergeladene !== null ? (q.regeln.nur_heruntergeladene ? "nur heruntergeladene" : "auch nicht heruntergeladene") : "Downloadstand aus den Einstellungen"}
          ...
          {#if bearbeiten?.id === q.id}
            ...
            <div class="row g-3">
              ...
              <div class="col-md-4"><label class="form-label" for="r-dl">Nur heruntergeladene</label><select class="form-select" id="r-dl" bind:value={regeln.nur_heruntergeladene}><option value="">aus den Einstellungen</option><option value="true">ja</option><option value="false">nein</option></select></div>
    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        <div class="row g-3">
          <div class="col-md-3"><label class="form-label" for="n-typ">Typ</label><select class="form-select" id="n-typ" bind:value={neu.typ}>{#each Object.entries(typen) as [k, t] (k)}<option value={k}>{t}</option>{/each}</select></div>
          <div class="col-md-3"><label class="form-label" for="n-name">Name</label><input class="form-control" id="n-name" bind:value={neu.name} placeholder="z. B. morf" /></div>
          ...
          <div class="col-12 d-flex gap-2 align-items-center flex-wrap">
            <button class="btn btn-outline-secondary" onclick={kanalPruefen} disabled={pruefen || !neu.basis_url || !neu.kanal_id}>{#if pruefen}<i class="fa-solid fa-circle-notch fa-spin"></i>{/if} Kanal prüfen</button>
            {#if kanal}
      ...
      <div class="card">
        ...
        <div class="m-tabelle-feld" style="max-height: 480px; border: 0">
          <table class="table table-sm table-hover mb-0 align-middle">
            <thead><tr><th>Titel</th><th>Datum</th><th class="text-end">Dauer</th><th>Art</th><th>Heruntergeladen</th><th>Würde aufgenommen</th><th>Bekannt</th></tr></thead>
            <tbody>
