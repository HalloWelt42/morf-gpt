---
titel: Dokumente: Bücher und eigene Texte
unterzeile: E-Books, Markdown und Text neben den Videos, in derselben Bibliothek.
kategorie: Bibliothek
symbol: fa-book
stichworte: dokumente, epub, e-book, markdown, text, eigener text, kapitel, hochladen
---

Dokumente sind die zweite Werkart der Bibliothek. Ein E-Book (EPUB), eine Markdown- oder Textdatei wird in Kapitel gelesen, in Stücke zerlegt und eingebettet, genau wie die Videos. Der Chat findet danach Stellen aus Büchern und Videos in derselben Suche und belegt sie nebeneinander: ein Video mit Folge und Zeitfenster, ein Dokument mit Kapitel.

## Hochladen

Unter Dokumente, Hochladen nimmst du EPUB, Markdown (.md) und Text (.txt) an, auch mehrere Dateien auf einmal. Aus einem EPUB kommen Titel, Autor, Sprache, Datum und die Kapitel aus dem Inhaltsverzeichnis; aus Markdown werden Überschriften mit # zu Kapiteln, aus Text entsteht ein Abschnitt. Ein Dokument braucht weder Audio noch Transkription noch Korrektur: Es steht nach dem Import sofort auf dem Fließband für Stückeln und Einbetten.

## Eigener Text

Mit Eigener Text legst du Notizen oder eigene Texte direkt in der Oberfläche an. Überschriften mit # eröffnen Kapitel. Der Text wird als Markdown-Datei abgelegt und wie ein hochgeladenes Dokument verarbeitet.

## Kapitel und Stücke

Die Stückelung läuft je Kapitel: Ein Stück überschreitet nie eine Kapitelgrenze, und der Kapiteltitel wird zum Thema des Stücks. In der Dokumentansicht zeigt die Kapitelliste, wie viele Stücke jedes Kapitel hat; die Leseansicht daneben zeigt den vollen Text. Ein Beleg im Chat springt genau in dieses Kapitel.

## Im Chat

Unter Filter wählst du, ob die Antwort aus allem, nur aus Videos, nur aus Dokumenten oder aus einem bestimmten Dokument kommt. "Höchstens je Werk" begrenzt die Stellen je Video oder Dokument, damit ein einzelnes Buch die Antwort nicht dominiert.

<div class="m-hinweis tipp"><i class="fa-solid fa-lightbulb"></i> Titel, Autor, Datum und Beschreibung lassen sich wie bei Videos von Hand pflegen; gepflegte Felder bleiben bei einem erneuten Import stehen. PDF folgt als nächste Art.</div>
